...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 27 15:07:21 2020
...
def Reduction_IC(SUB, showI=False):
  ...
  if verification(SUB):
    ...
    POS=(('R','L'),('B','F'),('D','U'))
    RES=nb.aff2axcodes(maps.affine, POS) #afiines
    POSR=(('L','R'),('B','F'),('D','U'))
    orin=nb.orientations.axcodes2ornt(RES, POSR)
    mapsA=nb.orientations.apply_orientation(mapsA, orin) #matrix
    
    comps=np.shape(mapsA)[3]
    print(str(comps)+' components')

    ABS_PATH=os.getcwd()
    ANT=ABS_PATH[:ABS_PATH.rfind('/')]    
    os.system('mkdir '+ANT+'/ResultsClassification')
    os.system('mkdir '+ANT+'/ResultsClassification/RCBP')
    ...
    for k in range(comps):
      ...
      cv2.imwrite(ANT+'/ResultsClassification/RCBP/Comp-'+str(k)+'-axi.png',axi)
      cv2.imwrite(ANT+'/ResultsClassification/RCBP/Comp-'+str(k)+'-cor.png',cor)
      cv2.imwrite(ANT+'/ResultsClassification/RCBP/Comp-'+str(k)+'-sag.png',sag)
...
def classificationIC_by_CNN(SUB, DirMod):
  #print('Leonel Mera Jimenez ..........................................................................................')
  #print(CNN)
  
  mod=get_Layer_4('Sagittal', DirMod)
  
  imaIC=Reduction_IC(SUB, showI=False)
  typeOfIC = mod.predict(imaIC[0])
  ...
  np.save(ANT+'/ResultsClassification/data.npy', noise)   
  file2 = open(ANT+"/ResultsClassification/auto_labels_noise.txt", "w")
  file2.write("./filtered_func_data.ica" + os.linesep)
  file2.write("[")
  for i in noise[:-1]:
    file2.write(str(i)+", ")
  file2.write(str(noise[-1])+"]")
  file2.close()
...
if __name__=="__main__":
  ...
  Subject='/home/lxmera/neuro3/output/workingdir/preproc/_subject_id_01_task_name_rest/_fwhm_4/Descomposition_ICA'
  DIR='/home/lxmera/neuro3/Codigos_py/LAYER4-C_COM-Saggital-0.h5'
  
  #Generar la clasificación
  Clas,_=classificationIC_by_CNN(Subject, DIR)
